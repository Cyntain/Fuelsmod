...
import com.cyntain.Fm.Block.blockMultiSmelterCore;
...
public class TileEntityMultiSmelterCore extends TileEntity implements ISidedInventory{
    ...
    public void invalidateMultiblock()
    {
        ...
        metadata = metadata & blockMultiSmelterCore.MASK_DIR;
    ...
    {
        int dir = (getBlockMetadata() & blockMultiSmelterCore.MASK_DIR);
        ...
        for(int horiz = -1; horiz <= 1; horiz++)    // Horizontal (X or Z)
        {
            for(int vert = -1; vert <= 1; vert++)   // Vertical (Y)
            {
                for(int depth = 0; depth <= 2; depth++) // Depth (Z or X)
                {
                    ...
                    if(blockId != ModBlock.heatproofBrickBlock.blockID)
                        return false;
                }
            }
        }
        ...
        for(int horiz = -1; horiz <= 1; horiz++)    // Horizontal (X or Z)
        {
            for(int vert = -1; vert <= 1; vert++)   // Vertical (Y)
            {
                for(int depth = 0; depth <= 2; depth++) // Depth (Z or X)
                {
                    ...
                    worldObj.setBlock(x, y, z, ModMultiBlock.multiSmelterDummy.blockID);
                    worldObj.markBlockForUpdate(x, y, z);
                    TileEntityMultiSmelterDummy dummyTE = (TileEntityMultiSmelterDummy)worldObj.getBlockTileEntity(x, y, z);
        ...
        for(int horiz = -1; horiz <= 1; horiz++)    // Horizontal (X or Z)
        {
            for(int vert = -1; vert <= 1; vert++)   // Vertical (Y)
            {
                for(int depth = 0; depth <= 2; depth++) // Depth (Z or X)
                {
                    ...
                    if(blockId != ModMultiBlock.multiSmelterDummy.blockID)
                        continue;
                    
                    worldObj.setBlock(x, y, z, ModBlock.heatproofBrickBlock.blockID);
                    worldObj.markBlockForUpdate(x, y, z);
                }
            }
        }
    ...
    public void updateEntity()
    {
        ...
        if(!this.worldObj.isRemote)
        {
            ...
            if(isActive == 0 && furnaceBurnTime > 0)
            {
                ...
                isActive = 1;
                metadata = (isActive << 3) | (metadata & blockMultiSmelterCore.META_ISACTIVE);
    ...
    {
        return "multismelter.container.multismeltercore";
    }
